...
void DQNStageBridge::generateOccupancyGridCB(const sensor_msgs::LaserScanConstPtr &scan)
{
  if( scan->ranges.size() < 1)
  ...
  else
    {
      ...
      int inv_res = 10; // 0.2
      int x = scan->range_max* inv_res, y = scan->range_max*inv_res, rad = scan->range_max*inv_res;
      int dir = 0 ;//(int)robotRot.data;
      bool boomed = false;
      // ROS_INFO("here is ok one");
      // ROS_INFO("laserMap size %d, %d", laserMap.size().height, laserMap.size().width);
      for( float ind = 0; ind < 360; ind++)
      {
        int i = ind + dir;
        for( int j = 0; j < rad; j++)
          if( scan->ranges.at(ind)*inv_res>=j)
          {
            int idx = 40 + int( j*cos( PI*i/180));
            int idy = 40 + int( j*sin( PI*i/180));
            if( (idx < 0 | idx >=80) | (idy<0 | idy >=80))
              continue;
            // ROS_INFO("x, y: %d, %d", idx, idy);
            laserMap.at<uchar>(idx, idy) = 255;
          }
        if( scan->ranges.at(ind) < scan->range_max){
          int idx = 40 + int( inv_res*scan->ranges.at(ind)*cos( PI*i/180));
          int idy = 40 + int( inv_res*scan->ranges.at(ind)*sin( PI*i/180));
          if( (idx < 0 | idx >=80) | (idy<0 | idy >=80))
              continue;
          laserMap.at<uchar>(idx, idy) = 0;
        }
        if( scan->ranges.at(ind) < 0.66)
          boomed = true;
      }
      // ROS_INFO("here is ok two");
      if( boomed)
      {
        std_msgs::Empty boom;
