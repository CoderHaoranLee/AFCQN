...
float reset_x_set [] = { 30,  60, 84, 126}; //, 14,   10,   32,   108, 144, 168
float reset_y_set [] = { 21,  23, 25,  14}; //, 38,   75,  114,    98,  42, 72
float reset_angle [] = { 0,   0,  0,   0}; //, 4.71, 4.71, 3.14, 3.14,   0, 1.27
...
void stgPoseUpdateCB( Model* mod, ModelRobot* robot)
{
  ...
  tf::Quaternion q;
  q.setRPY(0, 0, robot->resetPose.a);
...
void stgLaserCB( Model* mod, ModelRobot* robot)
{
  ...
  reset_pose.pose.orientation = tf::createQuaternionMsgFromYaw(robot->pos->GetPose().a);
...
bool rosResetSrvCB(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
{
  ...
  robot->resetPose.a = reset_angle[idx];
