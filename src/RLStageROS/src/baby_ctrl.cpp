...
float reset_x_set [] = {1, 2, 5, 5, 7, 1, 3, 4, 3, 4, 6, 7};
float reset_y_set [] = {1, 1, 1, 2, 2, 3, 3, 3, 4, 4, 4, 4};
void stgPoseUpdateCB( Model* mod, ModelRobot* robot)
{
  ...
  transform.setOrigin( tf::Vector3(1.0 - robot->resetPose.x, 1.0 - robot->resetPose.y, 0.0) );
  tf::Quaternion q;
  q.setRPY(0, 0, 0);
