...
class MemoryBuffer(object):
    ...
    def memorize(self, state, action, reward, done, new_state, last_path_length, error=None):
        """ Save an experience to memory, optionally with its TD-Error
        """

        experience = (state, action, reward, done, new_state, last_path_length)
        if(self.with_per):
    ...
    def sample_batch(self, batch_size):
        ...
        last_p_batch = np.array([i[5] for i in batch])
        return s_batch, a_batch, r_batch, d_batch, new_s_batch, last_p_batch, idx
